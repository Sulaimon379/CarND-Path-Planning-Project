...
vector<int> getLaneIds(int lane, json sensor_fusion) {
  vector<int> ids;
  for (int i = 0; i < sensor_fusion.size(); i++) {
    ...
    if (other_car_lane < 0 || other_car_lane > 2)//if (other_lane >= 0 || other_lane <= 2){}
      continue;
...
double nearestDist(vector<int> ids, json sensor_fusion, double check_dist, double car_s) {
  ...
  for (int id : ids) {
    ...
    double dist_start = abs(check_start_car_s-car_s);//fabs(check_start_car_s-car_s);
    if ( dist_start < nearest_dist){
      nearest_dist = dist_start;
    }

    double dist_end = abs(check_end_car_s-car_s);//fabs(check_end_car_s-car_s);
    if ( dist_end < nearest_dist){
      nearest_dist = dist_end;
    }
  }
  return nearest_dist;
}
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
        	...
        	// Main car's localization Data
          	...
          	// Previous path data given to the Plannerref_x
            ...
            double closest_speed = 49.5;
          	...
          	if (too_close){
                ...
                int good_lane=lane;
                double good_lane_cost=numeric_limits<double>::max();
                for (int side_lane: side_lanes) {
                  ...
                  cost += Normalise(2.0 * (avg_speed-49.5/avg_speed)) * 1000;

                  // collision cost
                  auto ids = getLaneIds(side_lane,sensor_fusion);
                  double nearest = nearestDist(ids, sensor_fusion, .02*prev_size, car_s);
                ...
                // when a car is close and speed our speed is greater than average
                if (good_lane == lane && (ref_vel > lane_speeds[lane] || ref_vel > closest_speed))
                  ref_vel -= 0.224;
          	...
          	//  list of spaced waypoints, evenly spaced at 30m
          	vector<double> ptsx;
          	vector<double> ptsy;
			...
			for(int i = 1; i <= 50-prev_size; i++)
			{
				...
				double N = (target_dist / (0.02 * ref_vel/2.24));//ref vel = 49
